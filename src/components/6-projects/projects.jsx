...
const projects = ({filterProjects, runNewVideo, setUserFavoriteList, favoriteItems, setFavoriteItems, arLanguage})=> {
  const [activeVid, setActiveVid] = useState([2]);
  // const [favoriteItems, setFavoriteItems] = useState({}); // Track favorite states


useEffect(()=> {
    const activeFav = localStorage.getItem("favoriteItems");
    setFavoriteItems(activeFav && activeFav.length > 0 ? JSON.parse(activeFav): []); // I use [{}] instead of [] to avoid deleting last item when refresh the page, so I put empty object as initial value.
  }, [])
...
//-----------------  Handle (setUserFavoriteList) & pass it to Header Favorite List -----------------
// useEffect to update favorites whenever favoriteItems changes, then Pass result to App then Header Component
  useEffect(() => {
    const updatedFavorites = Data.filter(video => favoriteItems[video.id]);
    setUserFavoriteList(updatedFavorites);
  }, [favoriteItems]); // Runs when favoriteItems changes
// Pass toggleFavorite function to Header component to handle remove item button
    ...
    <div className="projects flex">
      ...
      {filterProjects.map(i => {
        return (
          ...
          {/* card */}
          {/* <a  className={favoriteItems[i.id]? 'heart icon-heart': 'heart icon-heart-o'} 
              onClick={()=> toggleFavorite(i.id)}>
          </a> */}
            <div className='card-img'>
              <div  
              className={activeVid.includes(i.id) ? 'dark-layer flex activeVid' : 'dark-layer flex'} 
                    onClick={()=> {   
                      runNewVideo(i.embedId);
                      setActiveVid([]); //remove activation from cards before active new one
                      setActiveVid(vid => [...vid, i.id]);
                      console.log(i.id);
                      }}
                      >
